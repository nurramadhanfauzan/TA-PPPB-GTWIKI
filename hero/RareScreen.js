...
function RareScreen({ navigation }) {
    const [data, setData] = useState([]);

    useEffect(() => {
        const fetchData = async () => {
            await axios
            .get('https://api.npoint.io/66b68fb3be60185f1d69/rarehero')
    ...
    return (
        <View style={styles.container}>
            ...
            <FlatList
                ...
                data={data}
                renderItem={({ item }) => (
                    <TouchableOpacity onPress={() => navigation.navigate('Detail Rare Hero', { msg: item.id })}>
