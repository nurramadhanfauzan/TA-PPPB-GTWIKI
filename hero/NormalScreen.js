...
function NormalScreen({ navigation }) {
    const [data, setData] = useState([]);

    useEffect(() => {
        const fetchData = async () => {
            await axios
            .get('https://api.npoint.io/66b68fb3be60185f1d69/normalhero')
    ...
    return (
        <View style={styles.container}>
            ...
            <FlatList
                ...
                data={data}
                renderItem={({ item }) => (
                    <TouchableOpacity onPress={() => navigation.navigate('Detail Normal Hero', { msg: item.id })}>
