...
const DetailNormalScreen = ({ route }) => {
  const [data, setData] = useState([]);

    useEffect(() => {
        const fetchData = async () => {
          await axios
          .get('https://api.npoint.io/66b68fb3be60185f1d69/normalhero/' + (route.params.msg - 1))
          .then(res => setData(res.data))
          .catch((e) => alert("System ", e.message))
        };
    fetchData();
    }, []);
...
export default DetailNormalScreen;